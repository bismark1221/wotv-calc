...
export class BuilderCardComponent implements OnInit, AfterViewInit {
  ...
  openLoadModal(cardId) {
    this.simpleModalService.addModal(BuilderModalLoadComponent, { type: 'card', savedItems: this.savedCards[cardId], allowNew: true })
  ...
  openSaveModal() {
    this.simpleModalService.addModal(BuilderModalSaveComponent, { type: 'card', item: this.card })
      .subscribe((isSaved) => {
        if (isSaved) {
          this.savedCards = this.cardService.getSavedCards();
        }
  ...
  openLinkModal() {
    this.simpleModalService.addModal(SharedLinkModalComponent, { type: 'card', item: this.card });
  }
